...
kmax = 1000;
up0 = u0;
theta0 = [0*pi/180, 0*pi/180];
thetap0 = [0,0];
forca0 = u0';
alfa0 = [0,0];

xini = [theta0,forca0,thetap0];
xref =    [0*pi/180;0*pi/180];
...
uMaxF1 = polyval(Fcoef1,pwmLim(1)) - up0(1);
uMaxF2 = polyval(Fcoef2,pwmLim(1)) - up0(2);
uMinF1 = polyval(Fcoef1,pwmLim(2)) - up0(1);
uMinF2 = polyval(Fcoef2,pwmLim(2)) - up0(2);
...
for k = 1:kmax
    xAux = x([1:q,q+3:end],k) + [ruidoPa(:,k);ruidoVa(:,k)];

    % Saida real
    y(:,k) = C * x([1:q,q+3:end],k);
    % Saida medida
    ym(:,k) = C * xAux;
